...
typedef struct {
    const NML3ConfigData *l3cd_auto;
...
typedef struct {
    ...
    const NML3ConfigData *l3cd_gw_extern;
    const NML3ConfigData *l3cd_generic;
    const NML3ConfigData *l3cd_static;
                      ...
                      GAsyncReadyCallback callback)
{
    ...
    if (!priv->dbus.bus_name) {
        //XXX: check all callers that they properly handle this case.
        _LOGT("dbus: call %s (no service, skip)", method_name);
...
nm_vpn_connection_get_l3cd(NMVpnConnection *self)
{
    ...
    return NM_VPN_CONNECTION_GET_PRIVATE(self)->l3cd_combined;
}
...
static void
_l3cfg_update_l3cd(NMVpnConnection *self, const NML3ConfigData *const *p_l3cd)
{
    NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE(self);
    NML3Cfg *               l3cfg;
    int                     priority;
    ...
    nm_assert(p_l3cd);
    nm_assert(((char *) (gpointer) p_l3cd) > ((char *) (gpointer) priv));
    nm_assert(((char *) (gpointer) p_l3cd)
              < ((char *) (gpointer) priv) + sizeof(NMVpnConnectionPrivate));

    if (NM_IN_SET(p_l3cd,
                  &priv->ip_data_4.l3cd_auto,
                  &priv->ip_data_6.l3cd_auto,
                  &priv->l3cd_generic,
                  &priv->l3cd_static)) {
        l3cfg = priv->l3cfg_if;
        if (!l3cfg) {
        ...
        nm_assert(NM_IN_SET(p_l3cd, &priv->l3cd_gw_extern));
        l3cfg = priv->l3cfg_dev;
    }
    ...
    if (applied) {
        s_ip4 = nm_connection_get_setting_ip_config(applied, AF_INET);
        s_ip6 = nm_connection_get_setting_ip_config(applied, AF_INET6);
    }

    merge_flags = NM_L3_CONFIG_MERGE_FLAGS_NONE;

    if (p_l3cd == &priv->ip_data_4.l3cd_auto) {
    ...
    } else if (p_l3cd == &priv->ip_data_6.l3cd_auto) {
...
static void
_l3cfg_update_l3cd_all(NMVpnConnection *self)
{
    NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE(self);

    _l3cfg_update_l3cd(self, &priv->l3cd_gw_extern);
    _l3cfg_update_l3cd(self, &priv->l3cd_generic);

    _l3cfg_update_l3cd(self, &priv->l3cd_static);
    _l3cfg_update_l3cd(self, &priv->ip_data_4.l3cd_auto);
    _l3cfg_update_l3cd(self, &priv->ip_data_6.l3cd_auto);
}
...
static void
_l3cfg_notify_cb(NML3Cfg *l3cfg, const NML3ConfigNotifyData *notify_data, NMDevice *self)
{
    /* FIXME(l3cfg): should we do something here? */;
}
...
_set_ip_ifindex(NMVpnConnection *self, int ifindex, gboolean is_if)
{
    ...
    *p_ifindex = ifindex;
    ...
    if (l3cfg_old) {
        ...
        nm_l3cfg_commit_on_idle_schedule(l3cfg_old, NM_L3_CFG_COMMIT_TYPE_AUTO);
        g_signal_handlers_disconnect_by_func(l3cfg_old, G_CALLBACK(_l3cfg_notify_cb), self);
    }
        ...
        for (IS_IPv4 = 1; IS_IPv4 <= 0; IS_IPv4--) {
            ...
            if (priv->l3cfg_if) {
                priv->ip_data_x[IS_IPv4].ip_config =
                    nm_ip_config_new(IS_IPv4 ? AF_INET : AF_INET6, priv->l3cfg_if, TRUE);
            }
...
disconnect_cb(GObject *source, GAsyncResult *result, gpointer user_data)
{
    ...
    _LOGT("dbus: disconnected%s%s",
          NM_PRINT_FMT_QUOTED2(error, " (", error->message, " with success"));
}
...
vpn_cleanup(NMVpnConnection *self, NMDevice *parent_dev)
{
    ...
    nm_clear_l3cd(&priv->l3cd_gw_extern);
    nm_clear_l3cd(&priv->l3cd_generic);
    nm_clear_l3cd(&priv->l3cd_static);
    nm_clear_l3cd(&priv->ip_data_4.l3cd_auto);
    nm_clear_l3cd(&priv->ip_data_6.l3cd_auto);

    _l3cfg_update_l3cd_all(self);
}
...
_parent_device_create_l3cd(NMVpnConnection *self)
{
    NMVpnConnectionPrivate *priv                 = NM_VPN_CONNECTION_GET_PRIVATE(self);
...
set:
    if (nm_l3_config_data_equal(l3cd, priv->l3cd_gw_extern))
        return FALSE;

    nm_l3_config_data_reset_take(&priv->l3cd_gw_extern, g_steal_pointer(&l3cd));
...
_check_complete(NMVpnConnection *self, gboolean success)
{
    ...
    if (success) {
        if (!priv->generic_config_received
            || (priv->ip_data_4.enabled && !priv->ip_data_4.l3cd_auto)
            || (priv->ip_data_6.enabled && !priv->ip_data_6.l3cd_auto)) {
...
{
    return nm_ip_addr_set_from_variant(AF_INET6, addr, v, NULL);
}
...
_dbus_signal_state_changed_cb(NMVpnConnection *self, guint32 new_service_state)
{
    ...
    _LOGD("VPN plugin: state changed: %s (%d)",
...
_config_process_generic(NMVpnConnection *self, GVariant *dict)
{
    ...
    nm_l3_config_data_reset(&priv->l3cd_generic, l3cd);
...
_dbus_signal_ip_config_cb(NMVpnConnection *self, int addr_family, GVariant *dict)
{
    ...
    if (g_variant_lookup(dict,
                         ...
                         &v_b)
        && v_b) {
        if (priv->ip_data_x[IS_IPv4].l3cd_auto) {
            NMDedupMultiIter ipconf_iter;
            const NMPObject *route;

            nm_l3_config_data_iter_obj_for_each (&ipconf_iter,
                                                 priv->ip_data_x[IS_IPv4].l3cd_auto,
                                                 &route,
        ...
        if (IS_IPv4) {
            ...
            while (
                ...
                if (!ip6_addr_from_variant(dest, &route.r6.network))
                    continue;

                ip6_addr_from_variant(next_hop, &route.r6.gateway);
    ...
    nm_l3_config_data_seal(l3cd);
    nm_l3_config_data_reset(&priv->ip_data_x[IS_IPv4].l3cd_auto, l3cd);

    nm_l3_config_data_log(l3cd,
                          IS_IPv4 ? "vpn-auto-config-4" : "vpn-auto-config-6",
                          "vpn:    ",
                          LOGL_TRACE,
                          _NMLOG_DOMAIN);
                                 ...
                                 const char *const *secrets)
{
    ...
    _LOGD("VPN plugin: requested secrets; state %s (%d)",
          vpn_state_to_string_a(priv->vpn_state),
          priv->vpn_state);
...
nm_vpn_service_daemon_exec(NMVpnConnection *self, GError **error)
{
    ...
    _LOGI("starting: VPN service has PID %lld", (long long) pid);
    return TRUE;
}
...
nm_vpn_connection_activate(NMVpnConnection *self, NMVpnPluginInfo *plugin_info)
{
    ...
    priv->l3cd_static =
        nm_l3_config_data_new_from_connection(nm_netns_get_multi_idx(priv->netns),
                                              nm_vpn_connection_get_ip_ifindex(self, TRUE),
                                              connection);
    nm_l3_config_data_seal(priv->l3cd_static);
    nm_l3_config_data_log(priv->l3cd_static,
                          "vpn-static-config",
                          "vpn:    ",
                          LOGL_TRACE,
                          _NMLOG_DOMAIN);
...
finalize(GObject *object)
{
    ...
    nm_l3_config_data_unref(priv->l3cd_gw_extern);
    nm_l3_config_data_unref(priv->l3cd_generic);
    nm_l3_config_data_unref(priv->l3cd_static);
    nm_l3_config_data_unref(priv->ip_data_4.l3cd_auto);
    nm_l3_config_data_unref(priv->ip_data_6.l3cd_auto);
}
